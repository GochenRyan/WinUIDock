    ...
    public class UniqueNamer
    {
        ...
        public string Name(string desired)
        {
            ...
            if (m_names.Contains(desired))
            {
                ...
                Parse(desired, out root, out desiredMinNumDigits, out suffixNumber);

                // Clamp the desired number of digits (from the desired string) to be at least as many as our class
                // specifies, an no more than this class supports...
                desiredMinNumDigits = Math.Min(Math.Max(desiredMinNumDigits, m_minNumDigits), MaxMinNumDigits);
                ...
                for (int i = 1; ; ++i)
                {
                    ...
                    if (desiredMinNumDigits > 1)
                    {
                        int suffixLength = iAsString.Length;
                        if (suffixLength < desiredMinNumDigits)
                            result += Zeros.Substring(0, desiredMinNumDigits - suffixLength);
                    }
        ...
        public void Parse(string name, out string root, out int suffixDigits, out int suffixNumber)
        {
            root = name;
            suffixNumber = 0;
            suffixDigits = 1;
